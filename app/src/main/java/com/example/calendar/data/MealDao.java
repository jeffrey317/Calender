...
public interface MealDao {
    ...
    @Query("SELECT * FROM meals WHERE date BETWEEN :startDate AND :endDate AND userId = :userId")
    List<Meal> getMealsByDateRange(String userId, Date startDate, Date endDate);

    @Query("SELECT * FROM meals WHERE userId = :userId AND date >= :date AND date < datetime(:date, '+1 day') AND mealType = :type")
